...
- A `template_redirect` handler 302-redirects any front-end project request to
...
- One canonical URL per project, on the frontend. WordPress URLs 302-redirect
  there (temporary, since the frontend URL comes from configuration and can
  change), so WordPress never serves a competing page.
