...
On publish, WordPress can `POST /api/revalidate` (with a shared secret) to bust the `wpgraphql` tag, so updated content appears immediately instead of after the interval.
...
- Frontend → Vercel/Netlify/static host. A WordPress publish hook calls `POST /api/revalidate` to trigger on-demand revalidation (see Caching & revalidation).
