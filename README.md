...
- **Typed end to end.** The frontend consumes WPGraphQL through `graphql-codegen`, and CI validates every query against the live schema.
...
- **PHP 8.1+** and **Composer** (for the plugin's PHP dependencies and tests)
...
```bash
pnpm env:start
wp-env run tests-cli --env-cwd=wp-content/plugins/headless-portfolio composer test
```

## Continuous integration

GitHub Actions runs three jobs in [.github/workflows/ci.yml](.github/workflows/ci.yml):

- **php**: PHPCS (WordPress Coding Standards) and PHPStan. PHPUnit runs locally through wp-env (see the plugin README).
- **js**: Prettier, ESLint, TypeScript, Vitest, and the production build.
...
- **e2e**: boots WordPress with wp-env and runs Playwright against the built frontend, exercising the plugin's REST, GraphQL, and rendering end to end.
...
- **Frontend** deploys to a static or Node host such as Vercel or Netlify. A WordPress publish hook can call `POST /api/revalidate` with the shared secret to revalidate on demand.
